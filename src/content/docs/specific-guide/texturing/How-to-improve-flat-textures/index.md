...
![Original flat Wild Rift textures without shadows](./1.webp)

My edit:

![Edited textures with baked occlusion shadows applied](./2.webp)
...
![Arnold menu visible in the Maya toolbar](./3.webp)
...
![Windows Apps and Features list showing Maya](./23.webp)

Select Maya and click on “Change”.

![Maya entry selected with the Change button](./24.webp)

Now you get a Autodesk pop-up. Select the Arnold renderer (which is not installed in your case) and press install.

![Autodesk installer with the Arnold renderer selected](./25.webp)
</details>
...
![Right-click marking menu with Assign new material](./4.webp)

Select **“aiAmbientOcclusion”**.

![Material list with aiAmbientOcclusion selected](./5.webp)
...
![Arnold render preview showing soft contact shadows](./6.webp)

Next click on **Arnold – Utilities – Render Selection to Texture**.

![Arnold Utilities menu with Render Selection to Texture](./7.webp)
...
![Render to texture window with resolution and camera samples](./8.webp)
...
![Photoshop EXR import loading alpha channel as transparency](./9.webp)

If your file looks something like this means it worked:

![Occlusion map layer loaded on top of the texture](./10.webp)
...
![Occlusion layer set to Multiply with harsh shadows](./11.webp)
...
![Clipped Curves adjustment layer lightening the shadows](./12.webp)
...
![Texture with gray shadows next to the skin color](./13.webp)
...
![New clipped layer set to Color at lowered opacity](./14.webp)
...
![Rough skin-color brush strokes over the shadow areas](./15.webp)![Painted color layer blending into the occlusion map](./16.webp)

![Comparison of painted versus unpainted shadow color](./17.webp)
...
![Layer style dialog opened by double-clicking the layer](./18.webp)
...
![Blend If slider dragged left showing crunchy edges](./19.webp)
...
![Split Blend If arrows creating a smooth transition](./20.webp)
...
![Curves adjustment layer with the Medium Contrast preset](./21.webp)

Another thing you can try is blurring the occlusion map if some shadows appear too harsh using Gaussian Blur.

![Gaussian Blur softening harsh occlusion shadows](./22.webp)
