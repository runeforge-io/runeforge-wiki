...
Images live in the guide's own folder and are referenced with a relative path,
with a **descriptive alt text** rather than the filename:

```md
<!-- Good -->
![The Bind Skin options window with max influences set to 3](./bind-options.png)

<!-- Bad -->
![14.png](./14.png)
```

The leading `./` matters: it is what puts the image through the build's
optimizer. A path starting with `/` is served untouched.
...
Don't set a size. The site caps very tall screenshots on its own, and a raw
`<img>` tag opts the image out of the optimizer entirely.
...
![Skeleton before alignment](./before.png) ![Skeleton after alignment](./after.png)
```
...
<video width="480" controls>
  <source src="/guide-media/my-cool-guide/clip.mp4" type="video/mp4" />
</video>
```
